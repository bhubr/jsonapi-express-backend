...
function mapRecord(record, type) {
  const id = record.id;
  const attributes = Object.assign({}, kebabAttributes(record));
...
function dateToMySql(date) {
  ...
  return date.getUTCFullYear() + "-" + twoDigits(1 + date.getUTCMonth()) + "-" + twoDigits(date.getUTCDate()) + " " + twoDigits(thidates.getUTCHours()) + ":" + twoDigits(date.getUTCMinutes()) + ":" + twoDigits(date.getUTCSeconds());
};
