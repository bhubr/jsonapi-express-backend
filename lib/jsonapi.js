module.exports = function(config, generateJwt, checkJwtMiddleware, modelDescriptors) {
  ...
  let middlewares = require('./middlewares')(checkJwtMiddleware, modelRelationships, modelDescriptors);
  ...
  _.forOwn(modelHooks, (hooks, key) => {
    if(hooks !== undefined) {
      if(hooks.beforeSave !== undefined) {
        beforeSaveHooks[key] = hooks.beforeSave;
      }
      if(hooks.afterSave !== undefined) {
        afterSaveHooks[key] = hooks.afterSave;
      }
    }
  });
  ...
  function afterSave(table, attributes) {
    if(afterSaveHooks[table] === undefined) return Promise.resolve(attributes);
    return afterSaveHooks[table](attributes);
  }
  ...
  function getPermissions(user) {
    ...
    .then(permissionIds => queryBuilder.selectIn('permissions', permissionIds))
    .then(queryAsync)
