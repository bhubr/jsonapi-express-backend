module.exports = function(config, generateJwt, checkJwtMiddleware, modelDescriptors, queryAsync) {
  ...
  if(config.permissions.enabled === false) {
    deleteCallbacks.splice(0, 1);
    updateCallbacks.splice(2, 1);
  }

  router.delete('/:table/:id', deleteCallbacks);
  ...
  // define the home page route
  router.patch('/:table/:id', updateCallbacks);
  router.put('/:table/:id', updateCallbacks);
