...
module.exports = function(modelDescriptors, modelRelationships, queryAsync) {
  return {
    ...
    //   });
    //   lineLogger('init', this.typeToTableMap);
    // },

    mapType: function(type) {
      const table = this.mapModelTable[type];
      lineLogger('## mapType', type, table);
      if (!table) {
    ...
    createRecord: function(type, attributes) {
      ...
      // lineLogger('## createRecord', type, table, attributes);
    ...
    findAll: function(type) {
      ...
      .then(records => {
        // lineLogger('storeSqlStrategy.findAll', type, records.map(r => (r.id)));
        return records;
      })
    ...
    findRecordBy: function(type, where, multi) {
      lineLogger(type, where, multi);
      const table = this.mapType(type);
      const selectQuery = queryBuilder.selectWhere(table, where);
      lineLogger('## findRecordBy', type, selectQuery, queryAsync, '\n');
      return queryAsync(selectQuery)
    ...
    getGetterFunc: function(type, relKey, rel, rev) {
      ...
      // lineLogger('getGetterFunc', rel, rev, mapKey);
      const map = {
        ...
        'belongsTo.belongsTo': function(record1, relateeType) {
          // lineLogger('#record1', record1, rel.reverse + 'Id')
          let query = {};
    ...
    getGetterFuncMulti: function(type, relKey, rel, rev) {
      ...
      const map = {
        'hasMany.belongsTo': function(records, relateeType) {
          ...
          // lineLogger(records, relateeType, relKebabPlural, query);
        ...
        'hasMany.hasMany': function(records, relateeType) {
          ...
          lineLogger('hasMany.hasMany', fieldId1, fieldId2);
    ...
    findRelatees: function(type, record, relKey) {
      ...
      .catch(err => {
        lineLogger('## findRelatees err', type, record, relKey);
        throw err;
      });
    },
