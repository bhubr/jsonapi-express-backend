...
function extractBelongsToBelongsTo(relationship, inverseRelationship) {
  ...
  return function(relationshipData) {
    // return new Promise((resolve, reject) => {
      ...
      .then(relateeRecord => {
        console.log('pouet #1', relateeRecord, inverseRelationship.relatedModel, relateeRecord[inverseRelationship.column]);
      ...
      .then(relateeRelateeRecord => {
        console.log('pouet #2', relateeRelateeRecord);
      })
      .then(() => (true));
    // });
  };
...
function getExtractionFunc(relationship, inverseRelationship) {
  // console.log(relationship, inverseRelationship);
  const funcName = 'extract' + capitalizeFirst(relationship.type) + capitalizeFirst(inverseRelationship.type);
