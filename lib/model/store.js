  ...
  _.forOwn(modelInitializing, (hash, key) => {
    switch(hash.__type__) {
      ...
      case 'rel':
        ...
        newModel.relationships[key] = _.pick(hash, acceptedRelationshipOptions);
        break;
      case 'meta':
      // console.log(key, hash);
        if(acceptedModelOptions.indexOf(key) !== -1) {
          newModel.meta[key] = hash.value;
        }
        break;
      default:
    }
...
DataStore.prototype.meta = function(value) {
  const __type__ = 'meta'; // + this._modelInitializingName + '_' + this._registerData.attr;
  // options = _.pick(options || {}, acceptedModelOptions);
