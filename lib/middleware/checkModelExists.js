...
function checkModelExists(req, res, next) {
  ...
  if(! model) {
    const kebabSingular = naming.toSingular(kebabPlural);
    return next(errors.UnknownModel(
      'Model `{{camelSingular}}` not found in model definitions (file `{{kebabSingular}}.js`)',
      { camelSingular, kebabSingular }
    ));
  }

  // Save model name for later
  req[REQ_DATA_KEY].modelName = camelSingular;
  req[REQ_DATA_KEY].tableName = model._tableName;
  return next();
}
