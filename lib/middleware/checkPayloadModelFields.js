...
function hasMissingAttr(modelName, attributes) {
  ...
  for (let i = 0 ; i < _requiredAttrs.length ; i++) {
    const requiredAttr = _requiredAttrs[i];
...
function hasMissingRel(modelName, relationships) {
  const relationshipKeys = Object.keys(relationships);
  const { _requiredRels } = modelDescriptors[modelName];
  // required attrs for POST
  for (let i = 0 ; i < _requiredRels.length ; i++) {
    const requiredKey = _requiredRels[i];
    if(relationshipKeys.indexOf(requiredKey) === -1) {
      return requiredKey; // res.status(400).json({ error: 'Required field `' + requiredAttr + '` not found in payload' });
    }
  }
}

function checkPayloadModelFields(req, res, next) {
  if(req.method !== 'POST') { // This shouldn't be necessary
    throw new Error('checkPayloadModelFields is only for POST');
  }
  ...
  if(missingAttr = hasMissingAttr(modelName, attributes)) {
    missingKey = _.kebabCase(missingAttr);
    return next(errors.MissingField(
      "Required attribute `{{modelName}}.{{missingAttr}}`: `{{missingKey}}` not found in payload's `attributes`",
      { modelName, missingAttr, missingKey }
    ));
  }
  if(missingRel = hasMissingRel(modelName, relationships)) {
    missingKey = _.kebabCase(missingRel);
    return next(errors.MissingField(
      "Required relationship `{{modelName}}.{{missingRel}}`: `{{missingKey}}` not found in payload's `relationships`",
      { modelName, missingRel, missingKey }
    ));
  }
