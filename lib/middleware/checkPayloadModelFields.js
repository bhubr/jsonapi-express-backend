...
function checkPayloadModelFields(req, res, next) {
  ...
  if(missingAttr = hasMissingAttr(modelName, attributes)) {
    return next(errors.MissingField("Required field `{{missingAttr}}` not found in `attributes`",
      { missingAttr }
    ));
  }
  if(missingRel = hasMissingRel(modelName, relationships)) {
    return next(errors.MissingField('Required field `{{missingRel}}` not found in `relationships`',
      { missingRel }
    ));
  }
