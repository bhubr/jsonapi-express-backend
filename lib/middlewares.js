...
function getCheckResourceExists(queryAsync) {
  return function checkResourceExists(req, res, next) {
    const { camelSingular, tableName } = req.jsonapiData;
    const selectResourceQuery = queryBuilder.selectOne(tableName, req.params.id);
    console.log('# checkResourceExists', camelSingular, tableName, selectResourceQuery);
    return queryAsync(selectResourceQuery)
    .then(records => {
      if(records.length === 0) {
        return res.status(404).json({ error: 'resource of type "' + camelSingular + '" not found' });
      }
      req.jsonapiData.existingRecord = records[0];
      return next();
    }); 
  }
}
