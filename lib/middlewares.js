...
const permissionCheckers = {
  self: req => {
    // console.log(typeof req.params.id, typeof req.jwt.userId)
    // console.log('## permissions check', req.params.id, req.jwt.userId, parseInt(req.params.id, 10) === req.jwt.userId, '\n\n');
    return parseInt(req.params.id, 10) === req.jwt.userId;
  },
  fail: () => (false)
}

// function getCheckPermissions(queryAsync) {
  // return 
  function checkPermissions(req, res, next) {
    ...
    // console.log(req.body, req.method.toLowerCase());
    const permissionBase = req.body.data.type + ':' +
      opPerMethod[req.method.toLowerCase()] + ':';
    const matchingPermission = _.find(req.jwt.permissions, p => (p.includes(permissionBase))) || 'fail';
    ...
    if(! permissionCheckers[permissionModifier](req)) {
      return res.status(403).json({ error: 'Insufficient rights' });
    }
    // return queryAsync(queryBuilder.selectWhere('permissions', { name: requiredPermission + '%' }, 'LIKE'))
    // .then(records => (records.map(record => (record.name))))
    // .then(matchingPermissions => {
...
function getExtractReqRelationships(map) {
  ...
  return (req, res, next) => {
    ...
    if(map[table] === undefined) {
      ...
      // GOOD return res.status(500).json({ error: 'undefined relationships for ' + table })
