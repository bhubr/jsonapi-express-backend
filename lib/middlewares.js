...
function getCheckPermissions(modelRelationships, queryAsync) {
  return (req, res, next) => {
    try {
      ...
      const relationships = modelRelationships[kebabPlural];
  ...
  return (req, res, next) => {
    try {
      ...
      if(map[camelSingular] === undefined) {
        ...
        // req.jsonapiData.allAttributes = req.body.data.attributes;
        // return next();
      }
      const relationshipsForType = map[camelSingular];
      // console.log('extractReqRelationships #2', relationshipsForType, relationships);
      req.jsonapiData.deferredRelationships = {};
      ...
      _.forOwn(relationships, (payloadRelationship, payloadKey) => {
        ...
        if( mapEntry.type === 'belongsTo' ) {
          req.jsonapiData.relationshipAttributes[camelKey + 'Id'] = parseInt(relationshipData.id, 10);
        }
        else if(mapEntry.type === 'hasMany' && revEntry.type === 'hasMany') {
          const relateeTable = mapEntry.model;
          const { pivotTable, thisFirst } = getPivotTable(camelKey, tableName, mapEntry.reverse, relateeTable);
          const ids = _.map(relationshipData, entry => parseInt(entry.id, 10));
          req.jsonapiData.deferredRelationships[pivotTable] = { thisFirst, relateeTable, ids };
        }
      ...
      // console.log(req.jsonapiData.allAttributes, req.jsonapiData.relationshipAttributes, req.jsonapiData.deferredRelationships);
      next();
    } catch(e) {
      console.log(e);
      return res.status(500).json({ error: e.message });
