...
function getExtractReqRelationships(map, queryAsync) {
  ...
  return (req, res, next) => {
    // try {
      const { modelName, tableName } = req[REQ_DATA_KEY];
      // lineLogger('#getExtractReqRelationships', req.method, req.url, modelName, tableName);
      ...
      Promise.reduce(mappedRelationships, (carry, relItem) => {
        ...
        // lineLogger(camelKey, mapEntry.type, revEntry.type);
