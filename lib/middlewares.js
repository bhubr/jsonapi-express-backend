...
function jsonApi(req, res, next) {
  res.jsonApi = function(data) {
    ...
    // lineLogger('#jsonApi response for', req.method, req.url, JSON.stringify({ data }));
    ...
    try {
      ...
      const permissionCheckers = {
        ...
        mine: () => {
          ...
          lineLogger('mine #1', userRelationshipKey, ownerKey);
      ...
      if(! matchingPermission) {
        lineLogger('no matching permissions', req.jwt.permissions, permissionBase);
...
function convertAttrOld(req, res, next) {
  ...
  // lineLogger('#convertAttrOld', attributes);
  const create = req.method === 'POST';
  req.body.data.attributes = processAttributes( attributes, { create } );
  // lineLogger('#convertAttrOld', attributes, req.body.data.attributes);
  next();
}
  ...
  return (table, queryAsync) => {
    return record => {
      ...
      _.forOwn(relationshipsForType, (mapEntry, key) => {
        ...
        // lineLogger(mapEntry, key, revEntry);
        ...
        if(mapEntry.type === 'hasMany' && revEntry.type === 'hasMany') {
          ...
          lineLogger('## many2many', key, pivotTable, fieldId1, fieldId2, query);
          queries.push(query);
...
function getGetGetRelationshipsMulti(map) {
  return (camelSingular, queryAsync) => {
    return records => {
      ...
      // lineLogger('    # Map for type ' + camelSingular +': ', map[camelSingular]);
      let i = 0;
      ...
      _.forOwn(modelRelationships, (mapEntry, key) => {
        // lineLogger('    # i=' + i, mapEntry.model, mapEntry.reverse);
      ...
      .then(results => {
        ...
        results.forEach((result, resIdx) => {
          ...
          records.forEach((record, recIdx) => {
            ...
            if(thisResult) {
              // lineLogger('result not empty', resIdx, recIdx, pkName, record, thisResult);
...
function extractBelongsTo(idOrUndef, camelSingular, tableName, camelKey, mapEntry, revEntry, relationshipData, carry, queryAsync) {
  ...
  if( revEntry.type !== 'belongsTo' ) {
    ...
    // lineLogger('\n##belongsTo/belongsTo', camelSingular, mapEntry.relatedModel, camelKey, mapEntry.reverse);
    const relTable = naming.toTableName(mapEntry.relatedModel, true);
    const relKey = mapEntry.inverse + 'Id';
    let update2 = {};
    ...
    // lineLogger('#needsClear/selectPreviousQuery/clearPreviousQuery', hasToClearExistingRel, selectPreviousQuery, clearPreviousQuery);
    carry.deferredOneToOne.push({ relTable, key: relKey, id: relationshipData.id });

    lineLogger('#needsClear ? ', hasToClearExistingRel, selectPreviousQuery);
    return ! hasToClearExistingRel ?
      ...
      .catch(e => {
        lineLogger('#err in promise chain', e); throw e;
      })
...
function extractManyToMany(idOrUndef, camelSingular, tableName, camelKey, mapEntry, revEntry, relationshipData, carry, queryAsync) {
  ...
  // lineLogger('#extractManyToMany carry', carry);
  return Promise.resolve(carry);
}
...
function extractManyToOne(idOrUndef, camelSingular, tableName, camelKey, mapEntry, revEntry, relationshipData, carry, queryAsync) {
  ...
  // lineLogger('#extractManyToOne carry', carry, relTable, relationshipData, relateeIds);
...
function getExtractReqRelationships(map, queryAsync) {
  ...
  return (req, res, next) => {
    // try {
      const { modelName, tableName, relationships } = req[REQ_DATA_KEY];
      lineLogger('#getExtractReqRelationships', req.method, req.url, modelName, tableName, relationships);
      // let { relationships } = req.body.data;
      if(relationships ===  undefined) {
        relationships = {};
      }
      if(map[modelName] === undefined) {
        return res.status(500).json({ error: 'undefined relationships for ' + modelName });
      }
      const relationshipsForType = map[modelName];
      // req[REQ_DATA_KEY].deferredManyToMany = {};
      // req[REQ_DATA_KEY].deferredOneToOne = [];
      // req[REQ_DATA_KEY].relationshipAttributes = {};
      const mappedRelationships = Object.keys(relationships).map(key => {
        return { key, relationship: relationships[key] };
      });
      // lineLogger('#getExtractReqRelationships', relationshipsForType, mappedRelationships);
      ...
      Promise.reduce(mappedRelationships, (carry, relItem) => {
        ...
        lineLogger(mapEntry, mapEntry.relatedModel);
        var revEntry = map[mapEntry.relatedModel][mapEntry.inverse];
        var func = getFunc(mapEntry.type, revEntry.type);
        lineLogger('func args', req.params.id, modelName, tableName, camelKey, mapEntry, revEntry, relationshipData, carry, queryAsync);
        return func(req.params.id, modelName, tableName, camelKey, mapEntry, revEntry, relationshipData, carry, queryAsync);
      }, {
        deferredManyToMany: {},
      ...
      .then(reduced => {
        // lineLogger('#getExtractReqRelationships', reduced);
        req[REQ_DATA_KEY].allAttributes = Object.assign({}, req[REQ_DATA_KEY].attributes, reduced.relationshipAttributes);
      ...
      // lineLogger(req.body.data.relationships);
      // _.forOwn(relationships, (payloadRelationship, payloadKey) => {
        // lineLogger('#3', payloadRelationship, payloadKey);
        // lineLogger('#4', camelKey, mapEntry, revEntry);


        // lineLogger('#4', payloadRelationship, payloadKey);
      // });
      // req[REQ_DATA_KEY].allAttributes = Object.assign({}, req.body.data.attributes, req[REQ_DATA_KEY].relationshipAttributes);
      // lineLogger(req[REQ_DATA_KEY].allAttributes, req[REQ_DATA_KEY].relationshipAttributes, req[REQ_DATA_KEY].deferredManyToMany);
      // next();
    // } catch(e) {
    //   lineLogger(e);
    //   return res.status(500).json({ error: e.message });
...
function getCheckResourceExists(queryAsync) {
  return function checkResourceExists(req, res, next) {
    ...
    lineLogger('# checkResourceExists', camelSingular, tableName, selectResourceQuery);
