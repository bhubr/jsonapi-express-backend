...
module.exports = function(baseDir, config) {
  ({ keys, whitelistedUrls } = config.security);
  const privateKey = (fs.readFileSync(path.normalize(baseDir + '/' + keys.private))).toString();  // get private key
  const publicKey = (fs.readFileSync(path.normalize(baseDir + '/' + keys.public))).toString();  // get public key


  function generateJwt(user, permissions) {
    const userId = user.id;
    return new Promise((resolve, reject) => {
      
      const { username, email } = user;
      jwt.sign({ userId, username, email, permissions }, privateKey, { algorithm: 'RS256' }, function(err, token) {
  ...
  function checkJwt(token) {
    return new Promise((resolve, reject) => {
      ...
      jwt.verify(token, publicKey, function(err, decoded) {
        if(err) return reject(err);
        // lineLogger('JWT payload', decoded); // decoded payload
  ...
  function checkJwtMiddleware(req, res, next) {
    const whitelistedUrls = whitelistedUrls !== undefined ? whitelistedUrls : [];
