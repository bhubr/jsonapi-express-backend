...
module.exports = function(baseDir, config) {
  ...
  function checkJwt(token) {
    return new Promise((resolve, reject) => {
      const jwtTimeout = config.jwtTimeout !== undefined ? config.jwtTimeout : 36000;
      jwt.verify(token, publicKey, function(err, decoded) {
        if(err) return reject(err);
        // console.log('JWT payload', decoded); // decoded payload
  ...
  function checkJwtMiddleware(req, res, next) {
    ...
    .then(decoded => {
      req.jwt = decoded;
      console.log('decoded ok', req.jwt, decoded);
      next();
    })
